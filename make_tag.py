...
class _RawPDFText(_pyx_baseclasses.canvasitem):
    ...
    def bbox(self):
        return _pyx_bbox.bbox_pt(
            self.x_pt, self.y_pt, self.x_pt + self.width_pt, self.y_pt + self.height_pt
        )
...
class PDFText:
    ...
    def draw(self, cvs, x_cm, y_cm, text, r=0.0, g=0.0, b=0.0):
        ...
        pdf_code = ("q %.3f %.3f %.3f rg BT /%s %.1f Tf %.2f %.2f Td (%s) Tj ET Q") % (
            r,
            g,
            b,
            self.font,
...
def generateAprilTag(
    ...
    try:
        ...
        print(
            f"[ERROR]: Requested tag ID of {tagID} not available in the {tagFamililyData.chosenTagFamiliy} TagFamiliy"
        )
...
def generateAprilBoard(
    ...
    for y in range(0, n_rows):
        for x in range(0, n_cols):
            ...
            pos = (
                margx * 100 + x * (1 + tagSpacing) * tagSize + tagSpacing * tagSize,
    ...
    if drawLegend:
        ...
        # Place legend below the tag, right-aligned within the board bounds
        board_right = (
            margx * 100 + n_cols * (1 + tagSpacing) * tagSize + tagSpacing * tagSize
        )
        label_text = PDFText(font="Helvetica", size_pt=font_size)
        caption_text = PDFText(font="Helvetica", size_pt=font_size)

        caption_lines = [
            f"family={tagFamily}, tag_id={id} {n_cols}x{n_rows} tags",
            f"size={tagSize}cm and spacing={tagSpacing * tagSize:.2f}cm",
        ]
        text_max_len = max(caption_text.text_width_cm(line) for line in caption_lines)

        pos_center_axis = (board_right - page_margin, page_margin)
        pos_center_text = (
            board_right - page_margin - (arrow_length / 4) - text_max_len,
            page_margin + line_size,
        )

        # X axis arrow (pointing left, matching OpenCV detection)
        c.stroke(
            path.line(
                pos_center_axis[0],
                pos_center_axis[1],
                pos_center_axis[0] - arrow_length,
                pos_center_axis[1],
            ),
            [
                color.rgb.red,
        ...
        label_text.draw(
            c,
            pos_center_axis[0] - (arrow_length * 1.15),
            pos_center_axis[1] - 0.1,
            "x",
            r=1.0,
            g=0.0,
            b=0.0,
        ...
        c.stroke(
            path.line(
                ...
                pos_center_axis[1],
                pos_center_axis[0],
                pos_center_axis[1] + arrow_length,
            ),
            [
                color.rgb.green,
                deco.earrow(
        ...
        label_text.draw(
            c,
            pos_center_axis[0] - 0.1,
            pos_center_axis[1] + (arrow_length * 1.15),
            "y",
            r=0.0,
            g=0.5,
            b=0.0,
        ...
        for idx, line in enumerate(reversed(caption_lines)):
            caption_text.draw(
                c, pos_center_text[0], pos_center_text[1] + idx * line_size, line
            )
...
if __name__ == "__main__":
    ...
    aprilOptions.add_argument(
        ...
        dest="symmCorners",
        action="store_true",
        help="add black corner squares (default: %(default)s)",
    )
    aprilOptions.add_argument(
        "--draw-legend",
        default=False,
        dest="drawLegend",
        action="store_true",
        help="add legend with axes and caption (default: %(default)s)",
    )
